...
void delete_node(Gate *val)
{
    //find val in gates and remove
    Gate *temp = gates;
    Gate *prev = NULL;
    while(temp != NULL)
    {
    ...
    free(val->type);
    for(int i = 0; i < val->numInputs; i++)
    {
        free(val->inputs[i]);
    }
    free(val->inputs);
    for(int i = 0; i < val->numOutputs; i++)
    {
        free(val->outputs[i]);
    }
    free(val->outputs);
...
//  starting here
    for(Gate *temp8 = gates; temp8 != NULL; temp8 = temp8->next)
    {
        for(Gate *temp9 = temp8->next; temp9 != NULL; temp9 = temp9->next)
        {

            if(strcmp(temp8->type, "OR") == 0 && strcmp(temp9->type, "OR") == 0)
            {
                char *outputname = malloc(13 * sizeof(char));
                strcpy(outputname, temp8->outputs[0]);
                int found = 0;
                char *inputname = malloc(13 * sizeof(char));

                if(strcmp(temp9->inputs[0], outputname) == 0)
                {
                    found = 1;
                    strcpy(inputname, temp9->inputs[1]);
                }

                else if(strcmp(temp9->inputs[1], outputname) == 0)
                {
                    found = 1;
                    strcpy(inputname, temp9->inputs[0]);
                }

                if(found == 1)
                {
                    if(strcmp(temp8->inputs[0], inputname) == 0 || strcmp(temp8->inputs[1], inputname) == 0)
                    {
                        strcpy(temp9->inputs[0], temp8->inputs[0]);
                        strcpy(temp9->inputs[1], temp8->inputs[1]);
                        Gate *ref = temp8;
                        temp8 = temp8->next;
                        delete_node(ref);
                    }
    ...
    for(Gate *temp8 = gates; temp8 != NULL; temp8 = temp8->next)
    {
        for(Gate *temp9 = temp8->next; temp9 != NULL; temp9 = temp9->next)
        {
            for(Gate *temp10 = temp9->next; temp10 != NULL; temp10 = temp10->next)
            {
                if((strcmp(temp8->type, "AND") == 0 && strcmp(temp9->type, "AND") == 0 && strcmp(temp10->type, "OR") == 0) || (strcmp(temp8->type, "OR") == 0 && strcmp(temp9->type, "OR") == 0 && strcmp(temp10->type, "AND") == 0))
                {
                    //check if one of the inputs in tempa and tempb is the same
                    char *inputname1 = malloc(13 * sizeof(char));
                    char *inputname2 = malloc(13 * sizeof(char));
                    char *inputname3 = malloc(13 * sizeof(char));
                    int found = 0;
                    if(strcmp(temp8->inputs[0], temp9->inputs[0]) == 0)
                    {
                        strcpy(inputname1, temp8->inputs[1]);
                        strcpy(inputname2, temp9->inputs[1]);
                        strcpy(inputname3, temp8->inputs[0]);
                        found = 1;
                    }

                    else if(strcmp(temp8->inputs[0], temp9->inputs[1]) == 0)
                    {
                        strcpy(inputname1, temp8->inputs[1]);
                        strcpy(inputname2, temp9->inputs[0]);
                        strcpy(inputname3, temp8->inputs[0]);
                        found = 1;
                    }

                    else if(strcmp(temp8->inputs[1], temp9->inputs[0]) == 0)
                    {
                        strcpy(inputname1, temp8->inputs[0]);
                        strcpy(inputname2, temp9->inputs[1]);
                        strcpy(inputname3, temp8->inputs[1]);
                        found = 1;
                    }

                    else if(strcmp(temp8->inputs[1], temp9->inputs[1]) == 0)
                    {
                        strcpy(inputname1, temp8->inputs[0]);
                        strcpy(inputname2, temp9->inputs[0]);
                        strcpy(inputname3, temp8->inputs[1]);
                    ...
                    {
                        char *outputname = malloc(13 * sizeof(char));
                        strcpy(outputname, temp10->outputs[0]);
                        strcpy(temp10->inputs[0], inputname1);
                        strcpy(temp10->inputs[1], inputname2);
                        strcpy(temp10->outputs[0], temp9->outputs[0]);

                        Gate *newGate2 = malloc(sizeof(Gate));
                        newGate2->type = malloc(13 * sizeof(char));
                        strcpy(newGate2->type, temp8->type);
                        newGate2->numInputs = 2;
                        newGate2->numOutputs = 1;
                        newGate2->inputs = malloc(2 * sizeof(char*));
                        newGate2->outputs = malloc(1 * sizeof(char*));
                        newGate2->next = temp10->next;
                        newGate2->selectors = NULL;
                        newGate2->numSelectors = 0;
                        newGate2->hidden = 0;
                        newGate2->inputs[0] = malloc(13 * sizeof(char));
                        newGate2->inputs[1] = malloc(13 * sizeof(char));
                        newGate2->outputs[0] = malloc(13 * sizeof(char));
                        strcpy(newGate2->inputs[0], inputname3);
                        strcpy(newGate2->inputs[1], temp9->outputs[0]);
                        strcpy(newGate2->outputs[0], outputname);
                        free(outputname);
                        
                        temp10->next = newGate2;
    ...
    // //distributive law minimization
    // Gate *tempa = gates;
    // Gate *tempb = gates->next;
    // Gate *tempc = NULL;
    // Gate *prev = NULL;
    // if(tempb != NULL)
    //     tempc = gates->next->next;

    // while(tempb != NULL && tempc != NULL)
    // {
    //     if((strcmp(tempa->type, "AND") == 0 && strcmp(tempb->type, "AND") == 0 && strcmp(tempc->type, "OR") == 0) || (strcmp(tempa->type, "OR") == 0 && strcmp(tempb->type, "OR") == 0 && strcmp(tempc->type, "AND") == 0))
    //     {
    //         if((strcmp(tempa->outputs[0], tempc->inputs[0]) == 0 && strcmp(tempb->outputs[0], tempc->inputs[1]) == 0) || (strcmp(tempa->outputs[0], tempc->inputs[1]) == 0 && strcmp(tempb->outputs[0], tempc->inputs[0]) == 0))
    //         {
    //             //check if one of the inputs in tempa and tempb is the same
    //             char *inputname1 = malloc(13 * sizeof(char));
    //             char *inputname2 = malloc(13 * sizeof(char));
    //             char *inputname3 = malloc(13 * sizeof(char));
    //             int found = 0;
    //             if(strcmp(tempa->inputs[0], tempb->inputs[0]) == 0)
    //             {
    //                 strcpy(inputname1, tempa->inputs[1]);
    //                 strcpy(inputname2, tempb->inputs[1]);
    //                 strcpy(inputname3, tempa->inputs[0]);
    //                 found = 1;
    //             }

    //             else if(strcmp(tempa->inputs[0], tempb->inputs[1]) == 0)
    //             {
    //                 strcpy(inputname1, tempa->inputs[1]);
    //                 strcpy(inputname2, tempb->inputs[0]);
    //                 strcpy(inputname3, tempa->inputs[0]);
    //                 found = 1;
    //             }

    //             else if(strcmp(tempa->inputs[1], tempb->inputs[0]) == 0)
    //             {
    //                 strcpy(inputname1, tempa->inputs[0]);
    //                 strcpy(inputname2, tempb->inputs[1]);
    //                 strcpy(inputname3, tempa->inputs[1]);
    //                 found = 1;
    //             }

    //             else if(strcmp(tempa->inputs[1], tempb->inputs[1]) == 0)
    //             {
    //                 strcpy(inputname1, tempa->inputs[0]);
    //                 strcpy(inputname2, tempb->inputs[0]);
    //                 strcpy(inputname3, tempa->inputs[1]);
    ...
    //             if(found == 1)
    //             {
    //                 //DELETE GATES AND REPLACE WITH TWO GATES
    //                 Gate *newGate1 = malloc(sizeof(Gate));
    //                 newGate1->type = malloc(13 * sizeof(char));
    //                 strcpy(newGate1->type, tempc->type);
    //                 newGate1->numInputs = 2;
    //                 newGate1->numOutputs = 1;
    //                 newGate1->inputs = malloc(2 * sizeof(char*));
    //                 newGate1->outputs = malloc(1 * sizeof(char*));
    //                 newGate1->selectors = NULL;
    //                 newGate1->numSelectors = 0;
    //                 newGate1->hidden = 0;
    //                 newGate1->inputs[0] = malloc(13 * sizeof(char));
    //                 newGate1->inputs[1] = malloc(13 * sizeof(char));
    //                 newGate1->outputs[0] = malloc(13 * sizeof(char));
    //                 strcpy(newGate1->inputs[0], inputname1);
    //                 strcpy(newGate1->inputs[1], inputname2);
    //                 strcpy(newGate1->outputs[0], tempb->outputs[0]);

    //                 Gate *newGate2 = malloc(sizeof(Gate));
    //                 newGate2->type = malloc(13 * sizeof(char));
    //                 strcpy(newGate2->type, tempa->type);
    //                 newGate2->numInputs = 2;
    //                 newGate2->numOutputs = 1;
    //                 newGate2->inputs = malloc(2 * sizeof(char*));
    //                 newGate2->outputs = malloc(1 * sizeof(char*));
    //                 newGate2->next = tempc->next;
    //                 newGate2->selectors = NULL;
    //                 newGate2->numSelectors = 0;
    //                 newGate2->hidden = 0;
    //                 newGate2->inputs[0] = malloc(13 * sizeof(char));
    ...
    //                 newGate2->outputs[0] = malloc(13 * sizeof(char));
    //                 strcpy(newGate2->inputs[0], inputname3);
    //                 strcpy(newGate2->inputs[1], tempb->outputs[0]);
    //                 strcpy(newGate2->outputs[0], tempc->outputs[0]);

    //                 newGate1->next = newGate2;
    //             }
    ...
    printf("OUTPUTVAR %d", numOutput);
    for(int i = 0; i < numOutput; i++)
    {
        printf(" %s", outputvar[i]);
    }
    if(tt == 1)
    {
        printf("\n");
    ...
    {
        printf("%s ", tempf->type);
        if(strcmp(tempf->type, "MULTIPLEXER") == 0)
        {
            printf("%d ", tempf->numInputs);
        }
        for(int i = 0; i < tempf->numInputs; i++)
        {
            printf("%s ", tempf->inputs[i]);
        }
        if(strcmp(tempf->type, "MULTIPLEXER") == 0)
        {
            for(int i = 0; i < tempf->numSelectors; i++)
            {
                printf("%s ", tempf->selectors[i]);
            }
        }
        for(int i = 0; i < tempf->numOutputs; i++)
        {
            printf("%s ", tempf->outputs[i]);
        }
        printf("\n");
        tempf = tempf->next;
    }
